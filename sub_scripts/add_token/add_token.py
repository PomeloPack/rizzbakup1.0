...
ques2 = input(f"Are data in tokensadd.txt correct? after this confirmation tokens will be add!: (yes/no): ")
...
token_file = "/opt/fare/scripts/add_token/tokensadd.txt"
try:
    with open(token_file, 'r') as file:
        data_list = [line.strip() for line in file]

    for line in data_list:
        curl_command = f"curl -u mgmt:mgmt -X GET http://localhost:4151/api/management/blacklist/add/{code}/{line}"
